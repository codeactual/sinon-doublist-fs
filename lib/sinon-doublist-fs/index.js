...
function sinonDoublistFs(test) {
  ...
  fsStub = test.stub(fs);
  fsStub.Stats.restore(); // Restore constructor for fs.stat*/fs.lstat* stubbing
 ...
 *     ]);
 *
 * @param {array} paths File/directory absolute paths w/out trailing slashes
 */
mixin.stubTree = function(paths) {
  ...
  each(paths, function(name, readdir) {
    self.stubFile(name)
      .readdir(readdir.length ? readdir : false)
 ...
 * @param {function} cb
 */
customFsStub.exists = function(filename, cb) {
  cb(!!fileStubMap[filename]);
};
 ...
 * @return {boolean}
 */
customFsStub.existsSync = function(filename) {
  return !!fileStubMap[filename];
};
  ...
  if (isArray) {
    if (is.object(paths[0])) { // FileStub array
      ...
      paths.forEach(function(childName) {
        context.stubFile(name + '/' + childName).make();
      });
    }
...
FileStub.prototype.make = function() {
  ...
  fsStub.lstat.withArgs(name).yields(null, statsObj);
  fsStub.lstatSync.withArgs(name).returns(statsObj);
...
FileStub.prototype.unlink = function() {
  ...
  fsStub.lstat.withArgs(name).throws(err);
  fsStub.lstatSync.withArgs(name).throws(err);
  fsStub.readdir.withArgs(name).throws(err);
  fsStub.readdirSync.withArgs(name).throws(err);
