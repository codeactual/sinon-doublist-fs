...
var loggers = {}; // One logger per 'source', ex. 'FileStub#renameSync'
...
customFsStub.renameSync = function(oldPath, newPath) {
  ...
  log('fs#renameSync', '%s to %s done', oldPath, newPath);
};
...
FileStub.prototype.copyTree = function(newName) {
  ...
  log('FileStub#copyTree', '%s to %s done', oldName, newName);
};
...
FileStub.prototype.readdir = function(paths) {
  ...
  log('FileStub#readdir', '%s done', name);
