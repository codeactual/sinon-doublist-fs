...
var logger; // NodeConsole logger function
var loggerSource; // Current source, ex. 'FileStub#make'
...
FileStub.clone = function(srcStub) {
  ...
  logPush('FileStub.clone', srcName);
...
FileStub.prototype.map = function(cb) {
  var name = this.get('name');
  logPush('FileStub#map', name);
  var readdir = this.get('readdir');
  if (!readdir) { logPop(); return; }
 ...
 */
function log() {
  if (!module.exports.trace) { return; }
  nodeConsole.set('namespace', loggerSource);
  logger.apply(null, arguments);
}

/**
 * @param {string} source Ex. 'FileStub#make'
 * @param {mixed} args* util.format() arguments
 */
function logPush(source) {
  ...
  loggerSource = source;
  nodeConsole.set('namespace', loggerSource);
  logger.push.apply(null, [].slice.call(arguments, 1));
}
