...
customFsStub.renameSync = function(oldPath, newPath) {
  ...
  logPush('fs#renameSync', '%s to %s in parent %s', oldPath, newPath, parentName);
  ...
  logPop('fs#renameSync', '%s to %s done', oldPath, newPath);
};
...
FileStub.prototype.copyTree = function(newName) {
  ...
  logPush('FileStub#copyTree', '%s to %s', oldName, newName);
  ...
  logPop('FileStub#copyTree', '%s to %s done', oldName, newName);
};
...
FileStub.prototype.readdir = function(paths) {
  ...
  logPop('FileStub#readdir', '%s done', name);
...
FileStub.prototype.make = function() {
  ...
  logPush('FileStub#make', '%s with %d children', name, readdir.length);
  ...
  logPop('FileStub#make', '%s done', name);
};
...
FileStub.prototype.unlink = function() {
  ...
  logPop('FileStub#unlink', name, 'done');
};
...
/**
 * @param {string} source Ex. 'FileStub#renameSync'
 * @param {mixed} args* util.format() arguments
 */
function logPush(source) {
  if (!module.exports.log) { return; }
  nodeConsole.set('namespace', source);
  log.push.apply(null, [].slice.call(arguments, 1));
}
 ...
 * @param {mixed} args* util.format() arguments
 */
function logPop(source) {
  if (!module.exports.log) { return; }
  nodeConsole.set('namespace', source);
  log.pop.apply(null, [].slice.call(arguments, 1));
}
